...
* <<httpconnection,`HttpConnection`>>: Specialized connection for HTTP requests
* <<httpconnectionprovider,`HttpConnectionProvider`>>: HTTP-specific connection provider
...
The `defineApiKeyHttpConnectionProvider` function requires three parameters.
...
+
`attributes` is an object that defines the API key metadata.
+
[source]
----
...
The `defineBasicHttpConnectionProvider` function requires two parameters.
...
The `defineBearerHttpConnectionProvider` function requires two parameters.
...
The `defineOAuth2Connection` function requires three parameters.
