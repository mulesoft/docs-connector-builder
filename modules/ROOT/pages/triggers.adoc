...
The `inputMapper` function maps the trigger's input and the current watermark to the operation's input type:
...
Suppose the <<operation,`findCategoriesOperation`>> defines this input:
...
The `initialWatermark` function determines the starting watermark for the polling cycle. This can be a fixed value, a dynamic value (such as `now()`), or calculated by executing an operation:
