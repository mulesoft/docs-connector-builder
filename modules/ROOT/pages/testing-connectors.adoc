...
+
* Use a real connection to the API (requires a connection profile). See xref:connections.adoc#use-connection-profiles[Use Connection Profiles].
...
. Ask the agent to check what tests can be generated. For example, enter `What can I generate tests for here?`. This calls the `uc_get_test_generation_info` MCP tool, which returns the available modules (CCM, ACM, FCM) and reports whether tests already exist.
...
. For each module you want to test, tell the agent whether to use a mocked connection or a real connection. For real connections, you need a connection profile ID, so use the `uc_list_connection_profiles` MCP tool first to xref:connections.adoc#list-connection-profiles-with-mcp-tools[list connection profiles] to retrieve the ID.
. Confirm your inputs. This calls the `uc_generate_tests` MCP tool, which generates tests for the selected modules.
