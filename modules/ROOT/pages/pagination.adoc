...
You can define pagination without AI assistance, with AI assistance (available when using Cursor), or with MCP tools (available when using Cursor).
...
[[define-pagination-with-mcp-tools]]
=== Define Pagination with MCP Tools (Cursor)
...
. Ask the agent to add pagination. For example, enter `Add pagination to this operation`. This calls the `uc_add_pagination` MCP tool, which adds pagination to the operation.
. Enter the required inputs when prompted.
. Confirm your inputs. Connector Builder generates paginated operations.
