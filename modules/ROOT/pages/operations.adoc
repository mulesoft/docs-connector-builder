...
If you have already generated base connectivity and want to modify the set of operations included from your API specification or AI assistance, select *Generate* > *Regenerate Connector* and choose which operations to add or remove.

If you have already generated base connectivity and want to add operations that are not present in the API specification or AI assistance, update the API specification or AI input with the new operations, and then regenerate base connectivity.
...
Operation invocations can either succeed or fail. A successful invocation returns `ResultSuccess`, while a failure returns `ResultFailure`. The meaning of "failure" depends on the external service you are integrating with.
...
The `com::mulesoft::connectivity::decorator::Operation` module provide decorators to append additional code before or after an operation invocation. These decorators include:
...
HTTP operations are the most common type of operation, so they have the `com::mulesoft::connectivity::transport::Http` dedicated module. This module standardizes HTTP operation shapes and provides utilities for parameter encoding and request execution. Here is a summary of the module.
...
Here is an example of an HTTP operation with the `\http://www.mulesoft.com/user/{userId}` endpoint:
...
Here is an example of encoding parameters in an HTTP operation:
